...
def fetch_openalex_works(
    search_term: str, api_key: str, limit: int | None = None
) -> pl.DataFrame:
    if not search_term:
        raise NoneValueAtProp('Property "search_term" is required at that function.')

    if not api_key:
        raise OpenAlexApiKeyError(
            "OpenAlex API_KEY is needed to be passed as a property function"
        )
    ...
    session.headers.update({"User-Agent": "mailto:leonardobolfarini@gmail.com"})
...
def format_openalex_response(article):
    ...
    source_name = (
        source_obj.get("display_name") if isinstance(source_obj, dict) else None
    )
